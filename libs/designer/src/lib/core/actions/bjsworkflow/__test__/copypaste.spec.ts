...
describe('extendUpstreamNodeIdsForScopePaste', () => {
  ...
  it('adds the enclosing scope container and its upstream context when pasting inside a scope', () => {
    // Simulates pasting a Condition_inner into Condition_outer. The caller-provided
    // upstreamNodeIds is empty because there is no sibling inside Condition_outer yet.
    const state = buildState();
  ...
  it('extends via a subgraph parentId by walking up to the owning scope', () => {
    // When pasting is scoped to a subgraph (e.g. the "actions" branch of a Condition),
    // parentId is the subgraph node. We should still surface the enclosing scope + its upstream.
    const state = buildState();
    const nodeMap: Record<string, string> = { Init_Variable: 'Init_Variable', Condition_outer: 'Condition_outer' };
