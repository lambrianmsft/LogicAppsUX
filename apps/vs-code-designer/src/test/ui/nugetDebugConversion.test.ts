...
import { clearBlockingUI, pushDialogButtonWithRetry, sleep, waitForQuickInputAndType, waitForQuickInputReady } from './helpers';
...
interface TasksJson {
  tasks?: Array<{ label?: string; dependsOn?: string | string[] }>;
}
...
function assertNugetDebugFiles(entry: WorkspaceManifestEntry): void {
  ...
  for (const label of ['clean', 'build', 'clean release', 'func: host start']) {
  ...
  assert.ok(
    (launchJson.configurations ?? []).some(
      (configuration) =>
        configuration.type === 'logicapp' &&
        configuration.request === 'launch' &&
        configuration.funcRuntime === 'coreclr' &&
        configuration.isCodeless === true &&
        configuration.processId === undefined
    ),
    'NuGet launch.json should use the Logic Apps codeless NuGet launch configuration'
  );
}
...
async function startDebuggingWithoutHarnessCleanup(workbench: Workbench, driver: WebDriver): Promise<void> {
  console.log('[nugetDebugConversion] Starting debug without harness port cleanup...');
  await runCommandFromPalette(workbench, driver, 'Debug: Start Debugging');
}
...
describe('NuGet conversion debug lifecycle', function () {
  ...
  it('debugs again after bundle-to-NuGet conversion without manual func cleanup', async () => {
    ...
    await runCommandFromPalette(workbench, driver, CONVERT_COMMAND_TITLE);
    ...
    await startDebuggingWithoutHarnessCleanup(workbench, driver);
